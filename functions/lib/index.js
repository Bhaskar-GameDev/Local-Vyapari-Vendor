...
exports.onChatMessageCreated = exports.assertRecentAuth = exports.signOutEverywhere = exports.revokeDevice = exports.listMyDevices = exports.validateSession = exports.onProductReviewWrite = exports.onShopReviewWrite = exports.onShopProfileUpdate = exports.assignMerchantRole = exports.beforeUserSignedIn = exports.beforeUserCreated = exports.onUserCreate = exports.onUserCreated = exports.getCloudinarySignature = exports.resolvePhoneLoginEmail = exports.onNewOfferAdded = exports.aggregateProductViews = exports.checkExpiredOffers = exports.onInventoryUpdate = void 0;
...
function assertAppCheck(context) {
    if (process.env.ENFORCE_APP_CHECK === "true" && context.app === undefined) {
        throw new functions.https.HttpsError("failed-precondition", "This request did not originate from an authorized app.");
    }
}
...
exports.onUserCreate = exports.onUserCreated;
// ─────────────────────────────────────────────────────────────────────────────
...
function buildAuthClaims(existingClaims, userData, opts) {
    const roles = (userData && userData.roles) || {};
    // Legacy single-role field fallback.
    const isCustomer = roles.customer === true || (userData === null || userData === void 0 ? void 0 : userData.role) === "customer" || true; // every account is at least a customer
    const isMerchant = roles.merchant === true || (userData === null || userData === void 0 ? void 0 : userData.role) === "merchant";
    const activeRole = opts.activeRole ||
        (typeof existingClaims.activeRole === "string" ? existingClaims.activeRole : undefined) ||
        (userData === null || userData === void 0 ? void 0 : userData.activeRole) ||
        "customer";
    return Object.assign(Object.assign({}, existingClaims), { roles: { customer: isCustomer, merchant: isMerchant }, activeRole, customer: isCustomer, merchant: isMerchant, mfa: opts.mfaEnrolled, emailVerified: opts.emailVerified, 
...
function deviceFingerprint(ipAddress, userAgent) {
    return crypto
        .createHash("sha256")
...
async function recordDeviceAndMaybeAlert(uid, ipAddress, userAgent) {
    try {
...
exports.beforeUserCreated = functions.auth.user().beforeCreate((user) => {
    ...
    return {
        customClaims: buildAuthClaims({}, null, {
            ...
            mfaEnrolled: (((_b = (_a = user.multiFactor) === null || _a === void 0 ? void 0 : _a.enrolledFactors) === null || _b === void 0 ? void 0 : _b.length) || 0) > 0,
            activeRole: "customer",
        }),
    };
});
...
exports.beforeUserSignedIn = functions.auth.user().beforeSignIn(async (user, context) => {
    ...
    const mfaEnrolled = (((_b = (_a = user.multiFactor) === null || _a === void 0 ? void 0 : _a.enrolledFactors) === null || _b === void 0 ? void 0 : _b.length) || 0) > 0;
    return {
        customClaims: buildAuthClaims(user.customClaims || {}, userData, {
            emailVerified: user.emailVerified === true,
            mfaEnrolled,
        }),
    };
});
exports.assignMerchantRole = functions.https.onCall(async (data, context) => {
    assertAppCheck(context);
    if (!context.auth) {
    ...
    if (shopName.length < 2 || shopAddress.length < 3) {
        throw new functions.https.HttpsError("failed-precondition", "Please complete your shop profile (name and address) before becoming a merchant.");
    }
    ...
    if (!phoneVerified && !emailVerified) {
        throw new functions.https.HttpsError("failed-precondition", "Please verify your phone number or email before becoming a merchant.");
    }
    ...
    // Set new claims
    await admin.auth().setCustomUserClaims(uid, Object.assign(Object.assign({}, existingClaims), { roles: Object.assign(Object.assign({}, existingRoles), { merchant: true }), activeRole: "merchant", merchant: true }));
...
async function updateProductRating(productId, shopId) {
    var _a;
    ...
    if (!resolvedShopId) {
        ...
        const productDoc = await db.collection("products").doc(productId).get();
        if (productDoc.exists) {
            resolvedShopId = (_a = productDoc.data()) === null || _a === void 0 ? void 0 : _a.shopId;
        }
    }
    if (!resolvedShopId) {
...
exports.listMyDevices = functions.https.onCall(async (data, context) => {
    assertAppCheck(context);
    if (!context.auth) {
...
exports.revokeDevice = functions.https.onCall(async (data, context) => {
    ...
    const deviceId = typeof (data === null || data === void 0 ? void 0 : data.deviceId) === "string" ? data.deviceId : "";
    if (!deviceId) {
...
exports.assertRecentAuth = functions.https.onCall(async (data, context) => {
    assertAppCheck(context);
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Sign in required.");
    }
    const maxAgeSeconds = typeof (data === null || data === void 0 ? void 0 : data.maxAgeSeconds) === "number" ? data.maxAgeSeconds : 300;
    const authTime = Number(context.auth.token.auth_time || 0); // seconds since epoch
    const ageSeconds = Math.floor(Date.now() / 1000) - authTime;
    if (authTime === 0 || ageSeconds > maxAgeSeconds) {
        throw new functions.https.HttpsError("failed-precondition", "Please re-enter your credentials to continue.");
    }
