...
class _WelcomeSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _buildWelcomeSection(context, shopState);
  }
}
...
class _ProductsStatCard extends ConsumerWidget {
  const _ProductsStatCard();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return FadeInSlide(
      ...
      child: _StatCard(
        ...
        value: productsState.maybeWhen(
            data: (list) => list.length.toString(), orElse: () => '0'),
...
class _OffersStatCard extends ConsumerWidget {
  const _OffersStatCard();
...
class _TodayViewsStatCard extends ConsumerWidget {
  const _TodayViewsStatCard();
...
class _ProfileClicksStatCard extends ConsumerWidget {
  const _ProfileClicksStatCard();
    ...
    return Column(
      ...
      children: [
        ...
        FadeInSlide(
          ...
          slideOffset: 12,
          child: RatingBreakdownWidget(distribution: ratingDist),
        ),
      ],
    );
  }
...
Widget _buildWelcomeSection(
    BuildContext context, AsyncValue<ShopModel?> shopState) {
  return FadeInSlide(
...
Widget _buildChartSection(BuildContext context, AnalyticsModel analytics) {
  ...
  final hasData =
      analytics.daily.values.any((stat) => stat.views > 0 || stat.clicks > 0);
...
Widget _buildLowStockList(
  ...
  if (productsState.hasError) {
    return const Text('Error loading inventory.',
